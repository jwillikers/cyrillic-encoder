...
[,sh]
----
python3 -m pip install --user conan~=1
----

. Configure the default build and host profiles for Conan.

.. Create the Conan profiles directory.
+
[,sh]
----
mkdir -p ~/.conan/profiles/{clang,gcc}
----

.. Create some profiles.
The following profiles are for Fedora 38's default versions of GCC and Clang.
+
--
.~/.conan/profiles/gcc/13/relwithdebinfo.jinja
[,ini]
----
include::profiles/gcc/13/relwithdebinfo.jinja[]
----

.~/.conan/profiles/gcc/13/release.jinja
[,ini]
----
include::profiles/gcc/13/release.jinja[]
----
--

.. Configure the default Conan profiles and potentially some other Conan options.
+
.~/.conan/global.conf
[,ini]
----
core:default_build_profile = gcc/13/release.jinja
core:default_profile = gcc/13/relwithdebinfo.jinja
...
. Before running CMake, install the necessary dependencies with Conan, using the default Conan profiles.
+
[,sh]
----
conan install . --build outdated --build cascade --update
----
...
[,sh]
----
bash -c '. build/generators/conanbuild.sh && cmake --build --preset relwithdebinfo'
----
...
[,sh]
----
bash -c '. build/generators/conanbuild.sh && . build/generators/conanrun.sh && ctest --output-on-failure --preset relwithdebinfo'
----
...
[,sh]
----
bash -c '. build/RelWithDebInfo/generators/conanbuild.sh && cmake --build  --preset relwithdebinfo --target fix-format'
----
